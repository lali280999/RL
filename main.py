...
class grid_world_solver:
    def __init__(self,world,reward,gamma,time,transition_model):
        ...
        # #Area = row * col * time
        # self.k = 1/10**(len(str(self.row * self.col * self.time)))
        # self.e = self.k * self.row * self.col * self.time

        # Perimeter = 2(row +  col) * time
        # self.k = 1/10**(len(str(self.row * self.col * self.time)))
        # self.e = self.k * 2 * (self.row + self.col) * self.time
    ...
    def decay_rate(self,k):
        e = self.e*0.99 #self.e * 1/(1 + np.exp(-(self.row * self.col * self.time)*k))
        return e
    ...
    def makemove(self,csquare, nsquare): #accepts 2 valid squares and returns the square after the move
        ...
        if rand<=self.transition_model:
            return nsquare
        else: #2 spaces in the same direction
            rand=random.random()
            if rand>=0.5:
    ...
    def policyeg(self,square, e): #epsilon-greedy policy
        ...
        return action
    def act_explore(self,square, e): #this policy usually chooses the least explored action and is sometimes greedy
        rand=random.random()
        actions=self.megatable[(square[0],square[1])][0].copy()
        vals=self.megatable[(square[0],square[1])][1].copy()
        if rand<e:
            m=max(actions)
            alist=[i for i,x in enumerate(actions) if x==m]
            action=alist[random.randint(0,(len(alist)-1))]
            ...
            a=min(vals)
            vlist=[i for i,x in enumerate(vals) if x==a]
            action=vlist[random.randint(0,(len(vlist)-1))]
        return action

    def prob_explore(self,square, rfactor): #explores using weighted probability of each action corresponding to its q value
        rand=random.random()
        actions=self.megatable[(square[0],square[1])][0].copy()
        acp=[]
        for i in range(len(actions)):
            if abs(actions[i])<100:
                acp.append(rfactor+1/(1+np.exp(-actions[i])))
            elif actions[i]<0:
                acp.append(0)
            else:
                acp.append(rfactor+1)
        action=acp.index(random.choices(acp,weights=acp)[0])
        # mini=min(actions)
        # maxim=max(actions)
        # if not (maxim==mini):
        #     for i in range(len(actions)):
        #         actions[i]=np.exp(actions[i])
        #         #actions[i]=(actions[i]-mini)/(maxim-mini)
        #     if rand>0.2:
        #         action=actions.index(random.choices(actions,weights=actions)[0])
        #     else:
    ...
    #This one implements weighted probability policy
    def sarsaprob(self):
        count = 0
        ...
        a=grid_world_solver.prob_explore(self,s,self.e)
        while t.time()-t0<self.time:

            s_dash=grid_world_solver.makemove(self,s,grid_world_solver.getsquares(self,s)[a])
            if (t.time()-t0)/self.time<e_percent:
                a_dash=grid_world_solver.prob_explore(self,s_dash,self.e)
            else:
                a_dash=grid_world_solver.act_explore(self,s_dash,0)
            ...
            if self.world[s_dash[0]][s_dash[1]] != 0 and self.world[s_dash[0]][s_dash[1]] != 'S':
                s_dash = start.copy()
                count += 1
                self.e = grid_world_solver.decay_rate(self,1)#self.e*(1-(t.time()-t0)/self.time)
        ...
        self.plot()
        self.plotreward()
        #self.printdict(self.megatable)

    #This one is least action policy
    def sarsa_la(self):
        count = 0
        grid_world_solver.rsttable(self)
        start=grid_world_solver.searchworld(self,'S')
        ...
        t0=t.time()
        # a=grid_world_solver.policyeg(self,s,self.e)
        a=grid_world_solver.act_explore(self,s,self.e)
        while t.time()-t0<self.time:
            s_dash=grid_world_solver.makemove(self,s,grid_world_solver.getsquares(self,s)[a])
            if (t.time()-t0)/self.time<e_percent:
                a_dash=grid_world_solver.act_explore(self,s_dash,self.e)
            else:
            ...
            if self.world[s_dash[0]][s_dash[1]] != 0 and self.world[s_dash[0]][s_dash[1]] != 'S':
                s_dash = start.copy()
                count += 1
                self.e = grid_world_solver.decay_rate(self,1)
                self.meanreward.append(self.totreward/count)
                self.time_reward.append(t.time()-t0)
...
if __name__ == "__main__":
    ...
    default_gamma = 0.9
    default_time =5 
    default_source = 'intermediate.txt'
    default_reward = -0.1
